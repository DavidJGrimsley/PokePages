    ...
    (set, get) => ({
      ...
      toggleForm: async (dex: number, form: FormType) => {
        ...
        if (isOnline) {
          try {
            ...
            console.log('[TRACKER] sync PUT -> ', buildApiUrl(`legends-za-tracker/${dex}`), { userIdPresent: !!userId });
            ...
            const res = await fetch(buildApiUrl(`legends-za-tracker/${dex}`), {
              method: 'PUT',
      ...
      loadFromDatabase: async () => {
        try {
          ...
          const url = buildApiUrl(`legends-za-tracker?userId=${userId}`);
          ...
          if (error instanceof TypeError && error.message === 'Network request failed') {
            console.error('[TRACKER] Network error details:', {
              ...
              stack: error.stack,
              url: buildApiUrl('legends-za-tracker'),
              // url: buildApiUrl('legends-za-tracker'),
      ...
      syncWithDatabase: async () => {
        ...
        try {
          ...
          const res = await fetch(buildApiUrl(`legends-za-tracker?userId=${userId}`), {
            headers: authHeaders(), // include bearer
          });
          ...
          if (updates.length > 0) {
            ...
            const res2 = await fetch(buildApiUrl('legends-za-tracker/batch'), {
              method: 'POST',
              headers: authHeaders(), // include bearer
      ...
      processPendingUpdates: async () => {
        ...
        try {
          ...
          const res = await fetch(buildApiUrl('legends-za-tracker/batch'), {
            method: 'POST',
            headers: authHeaders(), // include bearer
            body: JSON.stringify({ userId, updates }),
          });
