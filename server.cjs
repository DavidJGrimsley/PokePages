...
const app = express();
const PORT = process.env.PORT || 3000;  // Use port 3000 to match working server
...
function getCounterFilePath(pokemonName) {
  const sanitizedName = pokemonName.toLowerCase().replace(/[^a-z0-9_-]/g, '-');
...
// Catch-all handler: send back the main app for any other routes
app.get('*', (req, res) => {
